...
export default function handler(req: NextApiRequest, resp: NextApiResponse) {
	if (req.method !== 'POST') {
		resp.status(400).json({ success: false, message: 'Invalid request method' });
		return;
	}
	if (typeof req.body !== 'object') {
		resp.status(400).json({ success: false, message: 'Invalid request body' });
		return;
	}
	try {
		const data = { ...req.body };
		...
		console.error(ex);
		resp.status(500).json({ success: false, message: ex.message });
	}
}
